return {
	...
	init = function()
		...
		map({ "n", "v" }, "<leader>mc+", "<cmd>Checkmate cycle_next<CR>", { desc = "Cycle todo item(s) to the next state" })
		map({ "n", "v" }, "<leader>mc-", "<cmd>Checkmate cycle_previous<CR>", { desc = "Cycle todo item(s) to the previous state" })
		...
		map({ "n", "v" }, "<leader>mcR", "<cmd>Checkmate remove_all_metadata<CR>", { desc = "Remove all metadata from a todo item" })
		map({ "n" }, "<leader>mca", "<cmd>Checkmate archive<CR>", { desc = "Archive checked/completed todo items (move to bottom section)" })
		map({ "n" }, "<leader>mcv", "<cmd>Checkmate metadata select_value<CR>", { desc = "Update the value of a metadata tag under the cursor" })
		map({ "n" }, "<leader>mc]", "<cmd>Checkmate metadata jump_next<CR>", { desc = "Move cursor to next metadata tag" })
		map({ "n" }, "<leader>mc[", "<cmd>Checkmate metadata jump_previous<CR>", { desc = "Move cursor to previous metadata tag" })
	end,
}
